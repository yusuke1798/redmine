...
protected
  ...
  def get_image_filename( attrname )
    #attrname_escape = File.join(File.dirname(attrname),  CGI.escape(File.basename(attrname)))
...
#    image_uri = File.join(File.dirname(image_uri),  CGI.escape(File.basename(image_uri)))
  ...
  def openHTMLTagHandler(dom, key, cell)
    ...
    when 'img'
      if !tag['attribute']['src'].nil?
        ...
        #end
        ### T.B.D ### TCPDF 5.0.000 ###
        #tag['attribute']['src'] = CGI.unescape(tag['attribute']['src'])
#        tag['attribute']['src'] = WEBrick::HTTPUtils.unescape(tag['attribute']['src'])
